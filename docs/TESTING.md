...
Detector tests cover forward/reverse terms, controlled misspellings, punctuation and Unicode normalization, explicit local negation, later positive assertions, non-insurance categories, source priority, and conflicting evidence. Payload tests cover restored decimal normalization, backward-compatible payloads, description line breaks, gallery caps/deduplication, malformed URL omission, attribute normalization, seller bounds, and required URL/status/category/metadata validation. Structured extraction fixtures cover representative named listing objects and ensure avatars, recommendations, adverts, and non-Facebook image hosts are excluded.
...
7. Open **View vehicle** and verify title, price, mileage, location, description, available listing photos, structured attributes, safe Facebook link, notes, and shortlist/review/dismiss actions.
